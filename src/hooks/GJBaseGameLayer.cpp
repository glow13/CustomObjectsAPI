...
public:
    ...
    void setupLayers() {
        ...
        if (CustomObjectsManager::get()->getCustomObjectsCount() == 0) return;
        auto sheet = CustomObjectsSheet::getCustomSpritesheetTexture();
        if (!sheet) {
    ...
    CCNode* parentForZLayer(int zLayer, bool blend, int parent, int ui) {
        if (parent != CUSTOM_PARENT_MODE || ui) return GJBaseGameLayer::parentForZLayer(zLayer, blend, parent, ui);
