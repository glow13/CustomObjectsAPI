 ...
 */
class $object(CustomPortalObject, EffectGameObject) {
public:
    bool init() override {
        ...
        if (!getConfig().hasMainSprite()) this->setDontDraw(true);
        if (!this->m_editorEnabled && getConfig().hasDetailSprite()) {
            auto parent = geode::cast::base_cast<cocos2d::CCSprite*>(GJBaseGameLayer::get()->m_game2LayerB0);
            this->m_portalBack = this->addInternalChild(parent, getConfig().getDetailSprite(), cocos2d::CCPoint(0, 0), -90);
        }

        this->bindObjectProperty(111, this->m_cameraIsFreeMode);
        this->bindObjectProperty(112, this->m_cameraEditCameraSettings);
        this->bindObjectProperty(113, this->m_cameraEasingValue, [this](){return this->m_cameraEditCameraSettings;});
        this->bindObjectProperty(114, this->m_cameraPaddingValue, [this](){return this->m_cameraEditCameraSettings;});
        this->bindObjectProperty(370, this->m_cameraDisableGridSnap);
